...
export default function Home() {
  ...
  const steps = [
    { icon: '📝', title: `${labels.mentor} assigns`, body: 'One task for the whole batch, published by 9 AM.' },
    { icon: '🧠', title: `${labels.student} takes the test`, body: 'In-app MCQs or pen and paper, with a timer.' },
    { icon: '📈', title: 'Instant analysis', body: 'Rank, percentile and weakest topics, batch-only.' },
    { icon: '📸', title: 'Upload proof', body: 'Snap the answer sheet in seconds from the phone.' },
    { icon: '✅', title: `${labels.mentor} ticks`, body: `${labels.streak} and ${labels.points} land the moment it is verified.` },
  ];
  const roles = [
    { href: '/login?role=student', icon: '🎓', title: labels.student, body: `See today's task, your ${labels.streak.toLowerCase()}, your ${labels.batch.toLowerCase()} rank and nothing else.`, cta: `Enter with Student ID` },
    { href: '/login?role=mentor', icon: '🧑‍🏫', title: labels.mentor, body: 'Assign in one tap, verify 50 uploads in 15 minutes, spot who is slipping.', cta: `${labels.mentor} login` },
    { href: '/login?role=admin', icon: '🧭', title: labels.admin, body: 'Every batch at a glance, plus full control over points, rules, labels and branding.', cta: `${labels.admin} console` },
  ];
    ...
    <div className="min-h-screen flex flex-col">
      <section className="bg-hero text-white relative overflow-hidden">
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 pt-8 pb-24 grid lg:grid-cols-[1.1fr_0.9fr] gap-12 items-center relative">
          <div className="rise">
            <Pill tone="accent" icon="⚕️">Mentor-led daily practice</Pill>
            ...
            <p className="mt-5 text-lg text-white/80 max-w-xl">
              {brand.appName} puts every {labels.student.toLowerCase()} in a {labels.batch.toLowerCase()} of 50 with one {labels.mentor.toLowerCase()}. Test, analyse, upload, get ticked, keep the {labels.streak.toLowerCase()} alive.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Button href="/login?role=student" variant="accent" size="lg">I&apos;m a {labels.student}</Button>
              <Button href="/login?role=mentor" variant="light" size="lg">I&apos;m a {labels.mentor}</Button>
            </div>
            <ul className="mt-10 flex flex-wrap gap-x-8 gap-y-2 text-sm text-white/70">
              <li><b className="text-white">50</b> per {labels.batch.toLowerCase()}, never mixed</li>
              <li><b className="text-white">1</b> task every day</li>
              <li><b className="text-white">0</b> passwords for students</li>
            </ul>
          </div>
          ...
          <div className="relative h-[420px] hidden sm:block" aria-hidden="true">
            <Card className="absolute left-0 top-4 w-64 rise-2" pad="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-display text-3xl font-extrabold text-brand-deep leading-none">12 days</div>
                  <div className="text-xs text-ink-2 mt-1">9 to the next milestone</div>
                </div>
              </div>
            </Card>
            <Card className="absolute right-0 top-28 w-72 rise-3" pad="p-4">
              <div className="flex items-center justify-between">
                <Pill tone="brand">Today&apos;s task</Pill>
                <span className="text-xs text-ink-3 tabular">closes 11:59 PM</span>
              </div>
              <div className="mt-2 font-display font-bold text-brand-deep">Neural Control &amp; Coordination</div>
              ...
              <div className="mt-1.5 text-[11px] text-ink-3">31 of 50 submitted in your {labels.batch.toLowerCase()}</div>
            </Card>
            <Card className="absolute left-8 bottom-6 w-72 rise-4" pad="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-bold text-brand-deep">Ticked by Dr. Anjali Rao</div>
                  <div className="text-xs text-ink-2">Analysis unlocked · <b className="text-accent-ink">+28 {labels.points}</b></div>
                </div>
              </div>
            </Card>
          </div>
        ...
        <Card pad="p-6 sm:p-8">
          <div className="flex items-end justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-ink-2">Five steps, one clear owner each. It takes a {labels.student.toLowerCase()} under an hour and a {labels.mentor.toLowerCase()} under fifteen minutes.</p>
            </div>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6 py-14">
        <h2 className="font-display text-2xl font-bold text-brand-deep text-center">Three doors, one product</h2>
